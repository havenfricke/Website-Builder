...
async function createImage(body) {
    const sql = 'INSERT INTO images (title, alt_text, content) VALUES (?, ?, ?)';
    await db.query(sql, [body.title, body.altText, body.content]);
    return { title: body.title, altText: body.altText, content: body.content };
}
